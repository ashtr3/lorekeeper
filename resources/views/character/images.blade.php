                ...
                <div class="row mb-3">
                    <div class="col-md-7">
                        <div class="text-center">
                            <a href="{{ $image->canViewFull(Auth::check() ? Auth::user() : null) && Storage::disk('images')->exists($image->imageDirectory . '/' . $image->fullsizeFileName) ? $image->fullsizeUrl : $image->imageUrl }}" data-lightbox="entry"
                                data-title="{{ $character->fullName }} [#{{ $image->id }}] {{ $image->canViewFull(Auth::check() ? Auth::user() : null) && Storage::disk('images')->exists($image->imageDirectory . '/' . $image->fullsizeFileName) ? ' : Full-size Image' : '' }}">
                                <img src="{{ $image->canViewFull(Auth::check() ? Auth::user() : null) && Storage::disk('images')->exists($image->imageDirectory . '/' . $image->fullsizeFileName) ? $image->fullsizeUrl : $image->imageUrl }}" class="image"
                                    alt="{{ $image->character->fullName }}" />
                            </a>
                        </div>
                        @if ($image->canViewFull(Auth::check() ? Auth::user() : null) && Storage::disk('images')->exists($image->imageDirectory . '/' . $image->fullsizeFileName))
