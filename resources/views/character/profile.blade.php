...
@section('profile-content')
    ...
    <div class="mb-3">
        <div class="text-center">
            <a href="{{ $character->image->canViewFull(Auth::check() ? Auth::user() : null) && Storage::disk('images')->exists($character->image->imageDirectory . '/' . $character->image->fullsizeFileName) ? $character->image->fullsizeUrl : $character->image->imageUrl }}"
                data-lightbox="entry" data-title="{{ $character->fullName }}">
                <img src="{{ $character->image->canViewFull(Auth::check() ? Auth::user() : null) && Storage::disk('images')->exists($character->image->imageDirectory . '/' . $character->image->fullsizeFileName) ? $character->image->fullsizeUrl : $character->image->imageUrl }}"
                    class="image img-fluid" alt="{{ $character->fullName }}" />
            </a>
        </div>
        @if ($character->image->canViewFull(Auth::check() ? Auth::user() : null) && Storage::disk('images')->exists($character->image->imageDirectory . '/' . $character->image->fullsizeFileName))
