...
@section('admin-content')
    {!! breadcrumbs(['Admin Panel' => route('admin.index'), 'Item Categories' => route('admin.data.item-categories.index')]) !!}
    ...
    <div class="text-right mb-3"><a class="btn btn-primary" href="{{ route('admin.data.item-categories.create') }}"><i class="fas fa-plus"></i> Create New Item Category</a></div>
    @if (!count($categories))
            ...
            <tbody id="sortable" class="sortable">
                @foreach ($categories as $category)
                    <tr class="sort-item" data-id="{{ $category->id }}">
                        ...
                        <td class="text-right">
                            <a href="{{ route('admin.data.item-categories.edit', $category->id) }}" class="btn btn-primary">Edit</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
